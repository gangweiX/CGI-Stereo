...
class feature_extraction(nn.Module):
    def __init__(self):
        ...
        self.feature_CFG_16 = nn.Sequential(BasicConv(64, 64, kernel_size=3, stride=2, padding=1),
                                            BasicConv(64, 128, kernel_size=3, stride=2, padding=1),)

        self.feature_CFG_8 = nn.Sequential(BasicConv(128, 64, kernel_size=3, stride=2, padding=1),                                        
                                           )
    ...
    def forward(self, x):
        ...
        feat_16 = self.feature_CFG_16(l2)
        feat_8 = self.feature_CFG_8(l3)
        return {"gwc_feature": gwc_feature, "features": [feat_16, feat_8]}
...
class Context_Geometry_Fusion(nn.Module):
    ...
    def forward(self, cv, feat):
        '''
        '''
        feat = self.feat(feat).unsqueeze(2)
...
class hourglass(nn.Module):
    def __init__(self, in_channels):
        ...
        self.CGF_16 = Context_Geometry_Fusion(in_channels*4, 128)
    ...
    def forward(self, x, features):
        ...
        conv4 = self.CGF_16(conv4, features[0])

        conv5 = F.relu(self.conv5(conv4) + self.redir2(conv2), inplace=True)
        conv5 = self.CGF_8(conv5, features[1])
...
class ACVNet(nn.Module):
    ...
    def forward(self, left, right):
        ...
        if not self.attn_weights_only:
            ...
            out1 = self.dres2(cost0, features_left["features"])
            out2 = self.dres3(out1, features_left["features"])
